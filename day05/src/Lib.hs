...
result :: IO ()
result = print $ run [] 0 initialProgram

systemID = 5

run :: [Int] -> Int -> [Int] -> [Int]
run output address memory
    | opcode address memory == 99   = output
    | opcode address memory == 1    = run output (address+4) (execute address memory (+))
    | opcode address memory == 2    = run output (address+4) (execute address memory (*))
    | opcode address memory == 3    = run output (address+2) (replaceNth (memory!!(address+1)) systemID memory)
    | opcode address memory == 4    = run (myread (address+1) memory (mode1 address memory) : output) (address+2) memory
    | opcode address memory == 5    = run output (jumpTo address memory (/=)) memory
    | opcode address memory == 6    = run output (jumpTo address memory (==)) memory
    | opcode address memory == 7    = run output (address+4) (compareAndSave address memory (<))
    | opcode address memory == 8    = run output (address+4) (compareAndSave address memory (==))
    | otherwise                     = [-1]
