...
run output address memory
    ...
    | opcode address memory == 4    = run (myread (address+1) memory (param1 address memory) : output) (address+2) memory
    | otherwise                     = [-1]

execute address memory fnc =
    let p1 = (param1 address memory)
        p2 = (param2 address memory)
        p3 = (param3 address memory)
        val1 = (myread (address+1) memory p1)
        val2 = (myread (address+2) memory p2)
        nth = (resultAddress address memory p3)
...
resultAddress :: Int -> [Int] -> Int -> Int
resultAddress address memory mode =
    memory!!(address+3)
