...
minimumDistance = minimum $ SL.filter (\dist -> dist /= 0) pointsToDistance

pointsToDistance = SL.map pointToDistance calculateIntersections

calculateIntersections = SL.intersect (SL.toSortedList $ path wire1) (SL.toSortedList $ path wire2)

pointToDistance point = (abs (fst point)) + (abs (snd point))

path wire = foldl addPointsToPath [(0,0)] wire

addPointsToPath acc pathLine = acc ++ (pathLineToPoints pathLine (last acc))

pathLineToPoints :: [Char] -> (Int, Int) -> [(Int, Int)]
pathLineToPoints ('R':distance) (xStart, yStart)  = [ (xStart + x, yStart) | x <- [1..(read distance)]]
pathLineToPoints ('L':distance) (xStart, yStart)  = [ (xStart - x, yStart) | x <- [1..(read distance)]]
pathLineToPoints ('U':distance) (xStart, yStart)  = [ (xStart, yStart + y) | y <- [1..(read distance)]]
pathLineToPoints ('D':distance) (xStart, yStart)  = [ (xStart, yStart - y) | y <- [1..(read distance)]]

wiretest1 = ["R8","U5","L5","D3"]
wiretest2 = ["U7","R6","D4","L4"]
